...
exports.start = function(){
  program
    ...
    .description('download a version of your application that corresponds to the git shortsha')
    .option("-i, --install", "Will also run the special dist_install.js script after download")
  ...
  function setup(env, options){
    console.log("distdown setup");
    
    var settings = fs.existsSync('./.distdownrc');
    ...
    settings = fs.existsSync('~/.distdownrc');
    ...
    function writeConfigFile(config, cb){
      ...
      fs.writeFile("",JSON.stringify(config, null, 4), function(err){
        if(err){
          console.log(err);
          cb(err);
        }else {
